...
キーカスタマイズができる IME では「Shift+Ctrl+変換キー」に「全確定」、「Shift+Ctrl+無変換キー」に「全消去」を割り当てることで動作を改善しています。
## 実行ファイル
...
さらに Hachiku設定で「キー設定利用」をオンにすれば、{確定}する際に選択範囲を消してしまうことがなくなります。
...
* 編集モードD+F+H、J+K+Q、J+K+G、J+K+V、J+K+Bは変換中かどうかを問いません。
