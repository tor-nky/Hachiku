...
### 2026年1月1日 v1.12.12
* 「でゅてぃ」(EJP→EMK)はEキーを上げないと入力できない [Issue #45](https://github.com/tor-nky/Hachiku/issues/45) のを修正

### 2024年3月23日 v1.12.11
* 旧MS-IMEで左右シフトから一時英数入力中、編集モードの記号を押した後にIMEオフになったり変換未確定になったりするのを修正
* サロゲートペアになるユニコードを固有名詞ショートカットなどで入力したとき、確定文字にならないことがあるのを修正
* 「文字出力後に IME の状態を検出しない時間」を設定できるように

### 2024年2月11日 v1.12.10
* Windows 11 のメモ帳に、動画再生をしながら新MS-IMEで入力しても表示が引っ掛かりにくくした
* 秀丸エディタがIMEキャンセルで異常終了することがあるのでディレイを調整

### 2024年1月24日 v1.12.9
* 割り込み禁止がすぐ解けることがあったのを修正  
Core 2 Duo でも1秒間に35打鍵の高速入力テストに合格しました。
(v1.10.4～1.12.8では第6世代Core iシリーズ以上が合格でした)

### 2023年11月29日 v1.12.8
* 定義 "[]{確定}{↑}"、"(){確定}{↑}" でかっこの中へ移動するため、確定に Shift+Ctrl+変換 でなくエンターを使う  
* エンター同時押しをシフトとして扱う際の挙動を修正

### 2023年11月15日 v1.12.7
* Windows 11 以降のメモ帳と新MS-IMEの組み合わせで出力が引っかかることがあったのをさらに改善  
しかし動画再生をしながらでは入力が引っかかるのをどうしても無くすことはできませんので、
そういう使い方をするのであればMS-IMEを従来型になるよう設定するか、他のIMEの利用を検討してください。
* 新MS-IMEで".{確定}"、",{確定}"後に予測入力窓が残るのを軽減 
* 編集モード「IMEキャンセル」でもESCキーの連続の間にディレイを入れる(文字と回数の分離に修正)  
秀丸エディタがIMEキャンセルの時に異常終了しなくなるのを期待しています。
