...
;	編集モードC+V+Lの動作後にはクリップボードは空になる。ダミーの空白も入らない。
...
kanaGroup := "KO"
	...
	SetKana( AL_小 | AL_け	,"xke"	)	; (ヶ)
	SetKana( AL_小 | AL_か	,"xka"	)	; (ヵ)
...
kanaGroup := ""	; グループなし
	...
	SetKana( KC_F | KC_G	,"{ひらがな}{全角}"	, NR)	; IME OFF
	SetEisu( KC_F | KC_G	,"{ひらがな}{全角}"	, NR)	; (ATOK)英語入力ON は "{ひらがな}{英数}")
...
kanaGroup := "1R"
	...
	SetKana2(KC_D | KC_F | KC_J		,"{↑}"		,"{↑}"		, R)	; ↑
	SetKana2(KC_D | KC_F | KC_M		,"{↓}"		,"{↓}"		, R)	; ↓
	SetKana( KC_D | KC_F | KC_I		,"#/"					, NR)	; 再
	SetKana2(KC_D | KC_F | KC_K		,"+{↑}"	,"+{↑}"	, R)	; +↑
	SetKana2(KC_D | KC_F | KC_COMM	,"+{↓}"	,"+{↓}"	, R)	; +↓
	SetKana( KC_D | KC_F | KC_O		,"{Del}"				, R)	; Del
	SetKana2(KC_D | KC_F | KC_L		,"+{↑ 7}"	,"+{↑ 7}"	, R)	; +7↑
	SetKana2(KC_D | KC_F | KC_DOT	,"+{↓ 7}"	,"+{↓ 7}"	, R)	; +7↓
	SetKana( KC_D | KC_F | KC_P		,"+{Esc 2}"			 , "ESCx3")	; 入力キャンセル
	...
	SetEisu2(KC_D | KC_F | KC_J		,"{↑}"		,"{↑}"		, R)	; ↑
	SetEisu2(KC_D | KC_F | KC_M		,"{↓}"		,"{↓}"		, R)	; ↓
	SetEisu( KC_D | KC_F | KC_I		,"#/"					, NR)	; 再
	SetEisu2(KC_D | KC_F | KC_K		,"+{↑}"	,"+{↑}"	, R)	; +↑
	SetEisu2(KC_D | KC_F | KC_COMM	,"+{↓}"	,"+{↓}"	, R)	; +↓
	SetEisu( KC_D | KC_F | KC_O		,"{Del}"				, R)	; Del
	SetEisu2(KC_D | KC_F | KC_L		,"+{↑ 7}"	,"+{↑ 7}"	, R)	; +7↑
	SetEisu2(KC_D | KC_F | KC_DOT	,"+{↓ 7}"	,"+{↓ 7}"	, R)	; +7↓
	SetEisu( KC_D | KC_F | KC_P		,"+{Esc 2}"			 , "ESCx3")	; 入力キャンセル
...
kanaGroup := "2R"
	...
	SetKana2(KC_C | KC_V | KC_J		,"{←}"		,"{←}"		, R)	; ←
	SetKana2(KC_C | KC_V | KC_M		,"+{←}"	,"+{←}"	, R)	; +←
	SetKana( KC_C | KC_V | KC_I		,"#/"					, NR)	; 再
	SetKana2(KC_C | KC_V | KC_K		,"{→}"		,"{→}"		, R)	; →
	SetKana2(KC_C | KC_V | KC_COMM	,"+{→}"	,"+{→}"	, R)	; +→
	SetKana( KC_C | KC_V | KC_O		,"^v"					)		; Paste
	SetKana( KC_C | KC_V | KC_L		,"{C_Clr}"				)		; コピークリア
	SetKana2(KC_C | KC_V | KC_DOT	,"+{← 7}"	,"+{← 7}"	, R)	; +←7
	SetKana( KC_C | KC_V | KC_P		,"^z"					)		; Undo
	SetKana( KC_C | KC_V | KC_SCLN	,"^y"					)		; Redo
	SetKana2(KC_C | KC_V | KC_SLSH	,"+{→ 7}"	,"+{→ 7}"	, R)	; +→7
	...
	SetEisu2(KC_C | KC_V | KC_J		,"{←}"		,"{←}"		, R)	; ←
	SetEisu2(KC_C | KC_V | KC_M		,"+{←}"	,"+{←}"	, R)	; +←
	SetEisu( KC_C | KC_V | KC_I		,"#/"					, NR)	; 再
	SetEisu2(KC_C | KC_V | KC_K		,"{→}"		,"{→}"		, R)	; →
	SetEisu2(KC_C | KC_V | KC_COMM	,"+{→}"	,"+{→}"	, R)	; +→
	SetEisu( KC_C | KC_V | KC_O		,"^v"					)		; Paste
	SetEisu( KC_C | KC_V | KC_L		,"{C_Clr}"				)		; コピークリア
	SetEisu2(KC_C | KC_V | KC_DOT	,"+{← 7}"	,"+{← 7}"	, R)	; +←7
	SetEisu( KC_C | KC_V | KC_P		,"^z"					)		; Undo
	SetEisu( KC_C | KC_V | KC_SCLN	,"^y"					)		; Redo
	SetEisu2(KC_C | KC_V | KC_SLSH	,"+{→ 7}"	,"+{→ 7}"	, R)	; +→7
...
kanaGroup := "2L"
	...
	SetKana( KC_M | KC_COMM | KC_W	,"　　　×　　　×　　　×{確定}{改行}"	)	; x   x   x
	SetKana( KC_M | KC_COMM | KC_S	,"【{確定}"								)	; 【
	SetKana( KC_M | KC_COMM | KC_X	,"】{確定}"								)	; 】
	SetKana( KC_M | KC_COMM | KC_E	,"{Home}{→}{End}{Del 2}{←}"			)	; セリフ接ぎ
	SetKana( KC_M | KC_COMM | KC_D	,"{Home}{→}{End}{Del 4}{←}"			)	; ト書き接ぎ
	SetKana( KC_M | KC_COMM | KC_C	,"]{確定}{改行}"						)	; 」次行
	SetKana( KC_M | KC_COMM | KC_R	,"{Home}{改行}　{←}"					)	; セリフ切り
	SetKana( KC_M | KC_COMM | KC_F	,"{Home}{改行}　　　{←}"				)	; ト書き切り
	SetKana( KC_M | KC_COMM | KC_V	,"]{確定}{改行}[{確定}"					)	; 」「
	SetKana( KC_M | KC_COMM | KC_T	,"〇{確定}"								)	; ○
	SetKana( KC_M | KC_COMM | KC_G	,"　　　"								)	; □□□
	SetKana( KC_M | KC_COMM | KC_B	,"]{確定}{改行}　"						)	; 」□

	SetEisu( KC_M | KC_COMM | KC_Q	,"｜{確定}"								)	; ｜
	SetEisu( KC_M | KC_COMM | KC_A	,"《{確定}"								)	; 《
	SetEisu( KC_M | KC_COMM | KC_Z	,"》{確定}"								)	; 》
	SetEisu( KC_M | KC_COMM | KC_W	,"　　　×　　　×　　　×{確定}{改行}"	)	; x   x   x
	SetEisu( KC_M | KC_COMM | KC_S	,"【{確定}"								)	; 【
	SetEisu( KC_M | KC_COMM | KC_X	,"】{確定}"								)	; 】
	SetEisu( KC_M | KC_COMM | KC_E	,"{Home}{→}{End}{Del 2}{←}"			)	; セリフ接ぎ
	SetEisu( KC_M | KC_COMM | KC_D	,"{Home}{→}{End}{Del 4}{←}"			)	; ト書き接ぎ
	SetEisu( KC_M | KC_COMM | KC_C	,"」{確定}{改行}"						)	; 」次行
	SetEisu( KC_M | KC_COMM | KC_R	,"{Home}{改行}　{←}"					)	; セリフ切り
	SetEisu( KC_M | KC_COMM | KC_F	,"{Home}{改行}　　　{←}"				)	; ト書き切り
	SetEisu( KC_M | KC_COMM | KC_V	,"」{確定}{改行}「{確定}"				)	; 」「
	SetEisu( KC_M | KC_COMM | KC_T	,"〇{確定}"								)	; ○
	SetEisu( KC_M | KC_COMM | KC_G	,"　　　"								)	; □□□
	SetEisu( KC_M | KC_COMM | KC_B	,"」{確定}{改行}　"						)	; 」□
